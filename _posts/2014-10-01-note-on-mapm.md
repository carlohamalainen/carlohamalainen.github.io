...
I recently wrote some code using <a href="http://hackage.haskell.org/package/wreq">wreq</a>
...
I reasoned that ``mapM`` would construct its result lazily, then ``head`` would force evaluation of just the first element of the list. This isn't the case, as <a href="http://stackoverflow.com/questions/3270255/is-haskells-mapm-not-lazy">explained here</a>.  The function ``mapM`` is basically equivalent to this: 
...
Interestingly, the definition for ``Stream`` looks very similar to the definition for ``Free``, which I used in an <a href="/2014/06/07/notes-on-free-monads">earlier post about free monads</a>:
