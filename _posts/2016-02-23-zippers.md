...
Intuitively, we would expect that ``unZipper . createZipper = id``, and we can check this using <a href="https://hackage.haskell.org/package/QuickCheck-2.8.2/docs/Test-QuickCheck.html">QuickCheck</a>. First, provide an ``instance`` of ``Arbitrary`` for our binary trees: 
