/* $Id: upNextController.js 82640 2011-11-21 19:11:29Z priya.ollapally $ */
...
NYTD.UpNext = (function () {
  ...
  var getPopUpEl = function (data) {
    ...
    var nextArticleData = (function (articles) {
      var canonical_links = $$('head link[rel=canonical]');
      
      if (canonical_links.length) {
        var currentUrl = canonical_links[0].readAttribute('href');
  ...
  var changeCursor = function(cursorType) {
		popUpEl.setStyle({
			cursor : cursorType
		});
  };
  ...
  var showPopUp = function () {
    ...
    if(getCookie('upnext') == 0) {
	    ...
	    popUpEl.select(toggleControl)[0].removeClassName('hidden');
	    ...
	    new Effect.Morph(popUpEl, {
	    	...
	    	afterFinish: function() {
	    		popUpEl.select(toggleControl)[0].addClassName('hidden');
	    	}
	    });
  ...
  var setupPopUp = function (data) {
    ...
    popUpEl.select(toggleControl)[0].observe('mouseover', function(e) {
    	popUpEl.select(toggleControl)[0].writeAttribute('src', img_upnext_ro);
    });
    
    popUpEl.select(toggleControl)[0].observe('mouseout', function(e) {
		popUpEl.select(toggleControl)[0].writeAttribute('src', img_upnext_rest);
    });
